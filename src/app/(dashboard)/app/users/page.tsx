  ...
  return <>
    ...
    {mayManageMembers ? <Card><CardHeader><h2>Roller, status och omfördelning</h2><Badge>{memberships?.length ?? 0}</Badge></CardHeader><CardContent><div className="grid">{(memberships ?? []).map((membership) => {
      ...
      return <form action={updateTenantMember} className="form-section form-stack" key={membership.user_id}>
        ...
        <fieldset className="form-section"><legend>Team som användaren ska leda vid rollen team_lead</legend>{availableTeams.map((team) => { const selected = (teamMembers ?? []).some((member) => member.user_id === membership.user_id && member.team_id === team.id && member.role === "manager"); return <label className="check-row" key={team.id}><input type="checkbox" name="team_ids" value={team.id} defaultChecked={selected} /> {team.name}</label>; })}</fieldset>
        ...
        <p className="muted">Roll och faktisk team-manager-behörighet ändras i samma databastransaktion. Vid paus eller borttagning stoppas nya listtilldelningar och öppet arbete kan omfördelas.</p>
