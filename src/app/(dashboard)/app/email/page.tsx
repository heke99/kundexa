...
export default async function EmailPage({ searchParams }: { searchParams: Promise<{ customer?: string; error?: string }> }) {
  ...
  // The page opens on `messages.read`; the action demands `messages.send`.
  // A viewer could reach the form and never get past the first line of it.
  ...
  return <>
    ...
    <div className="split-layout">
      ...
      {maySend ? <Card><CardHeader><h2><Send size={16} /> Nytt e-postmeddelande</h2></CardHeader><CardContent>
      ...
      </CardContent></Card> : <Card><CardHeader><h2>Nytt e-postmeddelande</h2></CardHeader><CardContent><p className="muted">Din roll kan läsa utskick men inte skicka. Be en teamledare eller administratör att skicka meddelandet.</p></CardContent></Card>}
    </div>
  </>;
}
