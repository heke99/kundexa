...
export async function POST(request: Request) {
  ...
  try {
    ...
    await admin.from("tenant_integrations").update({
      ...
      configuration: { ...configuration, last_test_status: success ? "success" : "error", last_tested_at: testedAt, last_error: success ? null : safeError, last_test_provider_message_id: success ? result.id : null },
    ...
    await admin.from("audit_logs").insert({
      ...
      after_data: { tested_at: testedAt, provider_message_id: success ? result.id : null, error: safeError },
    });
    if (!success) return apiJson(correlationId, { error: safeError ?? "resend_test_failed" }, { status: 409 });
    return apiJson(correlationId, { data: { status: "active", tested_at: testedAt, provider_message_id: result.id } });
  } catch (error) {
